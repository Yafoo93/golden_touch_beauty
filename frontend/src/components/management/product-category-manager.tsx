...
export type ManagementProductCategory = {
  id: string;
  name: string;
  slug: string;
  description: string;
...
function CategoryEditor({
  ...
  initial: ManagementProductCategory;
  onUpdated: (category: ManagementProductCategory) => void;
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const updated = await apiFetch<ManagementProductCategory>(
        `products/management/product-categories/${initial.id}/`,
        {
          method: "PATCH",
    ...
    <form className="service-category-card" onSubmit={save}>
      <header>
        <div>
          ...
          <strong>{initial.product_count} {initial.product_count === 1 ? "product" : "products"}</strong>
        </div>
        <span className={`status-badge status-badge--${initial.is_active ? "active" : "inactive"}`}>
      ...
      <div className="management-form__grid">
        ...
        <div className="management-form__wide">
          <TextAreaField name="description" label="Description" defaultValue={initial.description} rows={3} />
        </div>
      </div>
      <label className="management-form__toggle">
        <input type="checkbox" name="is_active" defaultChecked={initial.is_active} />
        <span><strong>Active category</strong><small>Inactive categories and their products are hidden publicly.</small></span>
      </label>
      <ValidationSummary errors={errors} />
      {success ? <p className="content-editor__success" role="status">{success}</p> : null}
      <div className="service-category-card__actions">
        {initial.product_count === 0 ? (
          <Button type="button" variant="outline" onClick={remove} loading={deleting} loadingLabel="Deleting...">Delete empty category</Button>
        ) : null}
...
export function ProductCategoryManager({
  ...
  initialCategories: ManagementProductCategory[];
}) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await apiFetch<ManagementProductCategory>(
        "products/management/product-categories/",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <div className="service-category-manager">
      <form className="management-form__section" onSubmit={create}>
        <div className="management-form__section-heading">
          ...
          <p>A stable shop-filter slug is generated automatically.</p>
        </div>
        <div className="management-form__grid">
          <FormField name="name" label="Category name" maxLength={150} required />
          <FormField name="display_order" label="Display order" type="number" min={0} max={32767} defaultValue={categories.length + 1} required />
          <div className="management-form__wide">
        ...
        <label className="management-form__toggle">
          ...
          <span><strong>Active category</strong><small>Available for product assignment and public shop filtering.</small></span>
        </label>
        <ValidationSummary errors={errors} />
        <div className="management-form__actions">
          <Button type="submit" loading={creating} loadingLabel="Creating...">Add category</Button>
        </div>
      </form>
      <section className="service-category-manager__list">
        <header><h2>Existing categories</h2><span>{categories.length} categories</span></header>
